...
class Metricor:
    ...
    def metric_new(self, label, score, plot_ROC=True, alpha=0.2, coeff=3):
        """input:
        ...
        # area under curve
        auc = metrics.roc_auc_score(label, score, max_fpr=0.1)
        # plor ROC curve
        if plot_ROC:
...
def generate_curve(y_true, y_score, slidingWindow):
    metricor_ = copy.deepcopy(Metricor)
    labels_original = copy.deepcopy(y_true)
    score_original = copy.deepcopy(y_score)
    (
        tpr_3d,
        fpr_3d,
        prec_3d,
